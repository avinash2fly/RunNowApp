...
export async function fetchWeatherForecast(
  ...
  // Collect all hours from today and tomorrow, then slice 4 starting from current hour
  const allHours: HourlyWeather[] = (data.forecast?.forecastday ?? [])
    .flatMap((day: any) => day.hour);

  const nowEpoch = Math.floor(Date.now() / 1000);
  const upcoming = allHours.filter((h: HourlyWeather) => h.time_epoch >= nowEpoch - 1800);
